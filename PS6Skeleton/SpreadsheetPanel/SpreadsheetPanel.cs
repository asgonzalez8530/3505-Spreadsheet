...
namespace SS
{
    ...
    public partial class SpreadsheetPanel : UserControl
    {
        ...
        public void GetSelectionLocation(out int x, out int y, out int width, out int height)
        {
            drawingPanel.GetSelectionLocation(out x, out y, out width, out height);
        }
        ...
        private class DrawingPanel : Panel
        {
            ...
            // The cells that are currently focused
            private List<Address> focusedCells;
            ...
            public DrawingPanel(SpreadsheetPanel ss)
            {
                ...
                focusedCells = new List<Address>();
                _ssp = ss;
            }

            public void Focus(int col, int row)
            {
            ...
            public void GetSelectionLocation(out int x, out int y, out int width, out int height)
            {
                x = LABEL_COL_WIDTH + (_selectedCol - _firstColumn) * DATA_COL_WIDTH + 1;
                y = LABEL_ROW_HEIGHT + (_selectedRow - _firstRow) * DATA_ROW_HEIGHT + 1;
                width = DATA_COL_WIDTH - 2;
                height = DATA_ROW_HEIGHT - 2;
            }
